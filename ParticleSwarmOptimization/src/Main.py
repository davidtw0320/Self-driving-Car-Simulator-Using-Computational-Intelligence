...
class Window():
	def __init__(self):
		...
		self.particleNumber = tk.IntVar()
		self.learningRate1 = tk.DoubleVar()
		self.learningRate2 = tk.DoubleVar()
		# column2
		self.J = tk.IntVar()
		...
		elements ={
		0:[{"labelName":"-PSO演算法參數-"}, {"labelName":"迭代次數:","textvarible":self.loopTimes},{"labelName":"粒子數量:","textvarible":self.particleNumber},{"labelName":"φ1:","textvarible":self.learningRate1},{"labelName":"φ2:","textvarible":self.learningRate2}],
	...
	def training(self):
		if self.loopTimes.get()==0 or self.particleNumber.get()==0 or self.J.get()== 0 or self.batchSize.get()== 0 or self.trainingDataFileName.get() == "":
			self.trainingStatus.set("不合法")
		else:
			self.trainingStatus.set("訓練中")
			self.RBFN = RBFN(loopTimes=self.loopTimes.get(), particleNumber=self.particleNumber.get(), learningRate1=self.learningRate1.get(), learningRate2=self.learningRate2.get(), trainingDataPath=self.trainingDataPath, hiddenLayerNeuronsNumber= self.J.get(), batchSize = self.batchSize.get())
			self.RBFN.train()
